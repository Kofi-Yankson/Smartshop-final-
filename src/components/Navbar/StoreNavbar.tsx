...
export default function StoreNavbar() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/store/dashboard/products?query=${encodeURIComponent(searchQuery)}`);
    }
  };
  ...
  return (
    <nav className="bg-blue-700 text-white py-4 px-6 flex justify-between items-center">
      ...
      <div className="space-x-4">
        <Link href="/storefacing" className="hover:underline">
          Home
        </Link>
        <Link href="/storefacing/dashboard/products" className="hover:underline">
        |  Products
        </Link>
        <Link href="/storefacing/dashboard/profile" className="hover:underline">
        |  Profile
        </Link>
